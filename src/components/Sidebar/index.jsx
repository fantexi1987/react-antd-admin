        ...
        if (menu.length > 0) {
          ...
          return (
            <Menu.Item key={'menu'+item.key}>
              {
                item.url ? <Link to={item.url}>{item.icon && <Icon style={{marginLeft:0}} type={item.icon} />}<span>{item.name}</span></Link> : <div>{item.icon && <Icon type={item.icon} />}<span>{item.name}</span></div>
              }
            </Menu.Item>
          )
        }
